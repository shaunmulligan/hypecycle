...
async def get_last_location(activity_id: int):
    # Get the last location of an activity
    sel_query = gps_readings.select().where(gps_readings.c.activity_id == activity_id).order_by(gps_readings.c.id.desc()).limit(1)
